    ...
    function load () {
      $ajax('/project/detail?id=' + id, 'get', '', (res) => {
        ...
        if (res.code === 1) {
          ...
          fileNoe.setAttribute('src', 'https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/' + resData.pic)
    ...
    mui('.upload').on('change', '#file', () => {
      ...
      $upload('/upload/fileuploadaws', 'post', formData, (res) => {
        ...
        if (res.code === 1) {
          ...
          fileNoe.setAttribute('src', 'https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/' + res.data.smallUrl)
          fileNoe.style.display = 'block'
        }
      })
    })
