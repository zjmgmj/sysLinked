(function ($$, doc, $) {
      ...
      resData.map((item, index) => {
        temp += `<li class="border-b-grey flex flex-between align-center member-item" data-id="${item.projectUserId}" data-index="${index}">
          <div class="avator border-blue radius-b50">        
            <img src="${item.userPic !== 'null' && item.userPic !== '' ? 'https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/' + item.userPic : 'images/nike.png'}" alt="sysLinked" />            
          </div>
  ...
  mui.ready(function () {
    ...
    mui('.member-list').on('tap', '.member-item', function () {
      ...
      mui.openWindow({
        url: '/projectMember/memberInfo.html',
        id: 'memberInfo'
      })
    })
    ...
    mui('body').on('tap', '#inviteMembers', function () {
      mui.openWindow({
        url: '/projectMember/inviteMembers.html',
        id: 'inviteMembers'
      })
    })
    mui('body').on('tap', '#batchImport', function () {
      mui.openWindow({
        url: '/projectMember/deptList.html',
        id: 'deptList'
      })
    })
  });
