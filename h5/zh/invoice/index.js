(function ($$, doc, $) {
  ...
  mui.ready(function () {
    ...
    mui('.popup').on('tap', '#addExpenses', function () {
      mui.openWindow({
        url: '/h5/zh/invoice/invoiceDetails.html?invoiceClass=2',
        id: 'invoiceDetails'
      })
      // loadingShow()
      // const file = this.files[0]
    ...
    mui('.popup').on('tap', '#category', function () { 
      mui.openWindow({
        url: '/h5/zh/invoice/category.html',
        id: 'category'
      })
    })
    ...
    mui('#swiperContent').on('tap', '.invoice-item', function () {
      ...
      mui.openWindow({
        url: `/h5/zh/invoice/invoiceDetails.html?id=${id}&invoiceClass=${invoiceClass}`,
    ...
    function uploadImg (formData, invoiceClass) {
      $upload('/upload/fileuploadaws', 'post', formData, (res) => {
        ...
        if (res.code === 1) {
          console.log(res)
          mui.openWindow({
            url: '/h5/zh/invoice/invoiceDetails.html?url=' + res.data.url + '&invoiceClass='+invoiceClass,
            id: 'invoiceDetails'
          })
        }
      })
