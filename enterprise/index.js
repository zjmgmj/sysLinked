(function ($$, doc, $) {
  ...
  function joinorglist () {
    $ajax('/org/joinorglist?userId=' + userId, 'get', '', function (res) {
      ...
      let temp = ''
      resData.map((item) => {
        let pic = '../images/icon_enterprise_grey.jpg'
        if (item.isdefault === 1) {
          pic = '../images/icon_enterprise_blue.jpg'
        }
        let orgusertype = '进入'
  ...
  mui.ready(function () {
    ...
    mui('body').on('tap', '#createBusiness', function () {
      // console.log(this)
      mui.openWindow({
        url: '/enterprise/createEnterprise.html',
        id: 'createEnterprise'
      })
    })
    ...
    mui('#enterpriseContent').on('tap', '.options-btn', function () {
      ...
      if (orgusertype === '0') {
        ...
        mui.openWindow({
          url: '/organizationMember/index.html?orgId=' + Number(node.attr('data-id')),
