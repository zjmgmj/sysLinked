  ...
  function loadData () {
    $ajax('/projectfolder/list?userId=' + createUserid + '&parentId=' + parentId + '&projectId=' + pid + '&page=' + page + '&size=' + size, 'get', '', (res) => {
      ...
      resData.map((item, index) => {
        console.log(item)
        let filePic = '../images/file_icon.jpg'
        let fileType = 'file'
        // let path = item.pic
        if (item.type === 1) {
          const imgSuffix = ['.jpg', '.png', '.jpeg']
          if (item.name.substr(-4) === '.pdf') {
            filePic = '../images/file_pdf.jpg'
            fileType = 'pdf'
          } else if (item.pic && (imgSuffix.indexOf(item.pic.substr(-4)) !== -1 || item.pic.substr(-5) == '.jpeg')) {
            fileType = 'img'
            filePic = 'https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/' + item.pic
          } else {
            fileType = 'office'
            filePic = '../images/file-word.png'
          }
        }
  ...
  mui.ready(function () {
    ...
    mui('#fileList').on('tap', '.file-content', function () {
      ...
      if (fileType === 'file') {
        ...
        mui.openWindow({
          url: '/file/index.html?parentId=' + id,
          id: 'fileChild'
        })
      } else if (fileType === 'office') {
        window.open(`https://view.officeapps.live.com/op/view.aspx?src=https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/${path}`, '_blank')
      } else if (fileType === 'img') {
        mui.openWindow({
          url: '/file/detail.html?path=' + path,
          id: 'fileChild'
        })
      }
    })
