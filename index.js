...
(function ($$, doc, $) {
  ...
  function load () {
    ...
    $ajax('/project/list?page=1&size=50&orgId=' + orgId + '&userId=' + userId, 'get', null, (res) => {
      ...
      resData.map((item) => {
        const temp = `<li class="store-txt" data-status="${item.status}" data-id="${item.id}" data-orgId="${item.orgId}">
          <div class="store-left flex flex-between align-center">
            <img class="store-pic" src="${item.pic !== 'null' && item.pic !== '' ? 'https://test-for-syslinked1.s3.cn-north-1.amazonaws.com.cn/' + item.pic : 'images/nike.png'}" alt="sysLinked" />            
            <p class="col">
              <span>${item.id}</span>
              <span>${item.name}</span>
            </p>
      ...
      if ($('.store-txt')) maxWidth = $('.store-txt')[0].clientWidth
      mui('#pullrefresh').pullRefresh().endPulldownToRefresh();
    })
  }
  ...
  mui.ready(function () {
    ...
    mui('#content').on('tap', '.setting', function () {
      ...
      const id = $(this).parents('.store-txt').attr('data-id')
      mui.openWindow({
        url: '/projectMember/createProject.html?id=' + id,
        id: 'project'
      })
      return false
    })
    ...
    mui('.edit-group').on('tap', '.create-project', function () {
      ...
      // })      
      mui.openWindow({
        url: '/projectMember/createProject.html',
        id: 'project'
      })
    })
  });
